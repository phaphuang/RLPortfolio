...
import os
from pgportfolio.marketdata.asset_list import get_data

fname = sorted(glob('WindIndexData/MarketData/*.csv'))
...
class CoinDataManager:
    ...
    def select_coins(self, start, end):
        ...
        if not self._online:
            ...
            coins = [os.path.basename(n).split(".")[0] for n in fname][:self._coin_number]
            print("Hello coins: ", coins)
...
def coin_data_manager_init_helper(config, online=True,
    ...
    if not download:
        ...
        print(get_feature_list(input_config["feature_number"]))
